...
public class CCILPPattern extends ILPPattern {

	public CCILPPattern(TGGRule rule) {
		super(rule);
	}
	...
	protected Collection<TGGRuleElement> getSignatureElements(TGGRule rule) {
		return Stream.concat(rule.getNodes().stream(), rule.getEdges().stream())
				.filter(e -> e.getDomainType() != DomainType.CORR || e.getBindingType() == BindingType.CONTEXT)
				.collect(Collectors.toCollection(LinkedHashSet::new));
	}

	@Override
	protected String getPatternNameSuffix() {
		return PatternSuffixes.CC_ILP;
	}
