...
class ApplicationStubTemplate {
	
	def getApplicationStub(TGG tgg){
		return '''
		...
		public class Application {
			...
			public static void main(String[] args) throws IOException {
				...
				Resource tggR = rs.getResource(URI.createFileURI("model/«tgg.name».tgg.xmi"), true);
			...
			private static void registerMetamodels(ResourceSet rs){
				...
				Resource corr = rs.getResource(URI.createFileURI("model/«tgg.name».ecore"), true);
				EPackage pcorr = (EPackage) corr.getContents().get(0);
				Registry.INSTANCE.put(corr.getURI().toString(), corr);
				Registry.INSTANCE.put("platform:/resource/CDToDoc/model/«tgg.name».ecore", pcorr);
				Registry.INSTANCE.put("platform:/plugin/CDToDoc/model/«tgg.name».ecore", pcorr);
