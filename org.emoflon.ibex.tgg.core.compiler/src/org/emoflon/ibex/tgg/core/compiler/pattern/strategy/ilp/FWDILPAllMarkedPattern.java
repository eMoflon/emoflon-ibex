...
public class FWDILPAllMarkedPattern extends ILPAllMarkedPattern {

	public FWDILPAllMarkedPattern(TGGRule rule) {
		super(rule);
	}
	...
	protected Collection<TGGRuleElement> getSignatureElements(TGGRule rule) {
		return Stream.concat(rule.getNodes().stream(), rule.getEdges().stream())
				.filter(e -> e.getDomainType() == DomainType.SRC && e.getBindingType() == BindingType.CREATE)
				.collect(Collectors.toSet());
	}

	@Override
	protected String getPatternNameSuffix() {
		return PatternSuffixes.FWD_ILP_ALL_MARKED;
	}
