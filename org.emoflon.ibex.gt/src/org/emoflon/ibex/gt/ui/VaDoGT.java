...
//	private MouseEvent last;		//Used for camera movement -> not implemented
	...
	protected static String styleSheet = "edge {" + "size: 2px;" + "fill-color: black;" + "fill-mode: dyn-plain;"
			+ "text-alignment: under;" + "text-background-mode: plain;" + "}"

			+ "node {" + "size-mode: fit;" + "shape: box;" + "fill-color: grey, red, blue, green;"
			+ "stroke-mode: plain;" + "stroke-color: black;" + "stroke-width: 1.5px;" + "text-mode: normal;"
			+ "text-background-color: grey;" + "padding: 3px, 3px;" + "text-background-mode: plain;"
			+ "fill-mode: dyn-plain;" + "}";
	 ...
	 * @param stateManager                   Statemanager with all states and helper
	 *                                       functions
...
//		}
	...
	/**
	 * Prepares data set for plotting e.g. adds data for respective amount of states
	 * to the data set
	 */
...
//	}
	...
	/**
	 * Creates the nodes of the graph. Nodes parenting other nodes indicate edges
	 * between these objects recursive call to start at the bottom of the parenting
	 * hierarchy
	 * 
	...
	private List<EObject> createNodesFromList(List<EObject> list) {
		...
		for (EObject node : list) {
			...
			if (!nodeMap.keySet().contains(node)) {
				...
				// If node has children add all of them to a list and call createNodesFromList
				// recursive
	...
	private void UpdateGraphForwards() {
		...
		if (!localStateManager.getCurrentState().getChildren().isEmpty()) {
			...
			// curState.setText("Current State: " +
			// Integer.toString(localStateManager.modelStates.getStates().indexOf(currentRuleState)));
			// Create new nodes
					...
					if (node.equals(attributeDelta.getObject())) {
						for (EAttribute attribute : node.eClass().getEAttributes()) {
							if (attributeDelta.getAttribute().equals(attribute)) {
								node.eSet(attribute,
										localStateManager.getObjectFromValue(attributeDelta.getNewValue()));
							}
...
//		curState.setText("Current State: 0");
	...
	private void setMatchRuleInfo() {
		...
		if (!localStateManager.getCurrentState().getChildren().isEmpty()) {
			RuleState nextRuleState = localStateManager.getCurrentState().getChildren().get(0);
...
//				matchStream.remove(nextRuleState.getMatch());
	...
	private void showAllMatches() {
		if (!localStateManager.getCurrentState().getChildren().isEmpty() && !allMatchesShown) {
			allMatchesShown = true;
			RuleState nextRuleState = localStateManager.getCurrentState().getChildren().get(0);
			List<IBeXMatch> matchStream = calculateCurrentMatches(nextRuleState,
					nextRuleState.getMatch().getPatternName());
...
//			}
	...
	private void showAttributes(String nodeID) {
		...
		if (node != null) {
			...
			for (EAttribute attribute : node.eClass().getEAllAttributes()) {
				...
				// No need to check if next state is available because if info node exist this
				// is already true
				if (infoNode) {
					for (AttributeDelta delta : localStateManager.getCurrentState().getChildren().get(0)
							.getAttributeDeltas()) {
	...
	/**
	 * Colors and labels all nodes and edges effected by the apply of the current
	 * rule Structural changes are indicates by coloring the whole node / edge
	 * Changes of attributes are indicated by info nodes Green / ++: creation or
	 * change Red / --: Deletions in current state are not displayed because the
	 * objects do no exist anymore
	 */
		...
		if (localStateManager.getCurrentState().isInitial()) {
			...
			for (EObject newNode : currentRuleState.getStructuralDelta().getCreatedObjects()) {
				if (!nodeBlacklist.contains(newNode)) {
					nodeMap.get(newNode).setAttribute("ui.style",
							"fill-color: rgba(0,150,0,255); text-background-color: rgba(0,150,0,255);");
					nodeMap.get(newNode).setAttribute("ui.label",
							nodeMap.get(newNode).getAttribute("ui.label") + " ++");
				}
			}
	 ...
	 * Structural changes are indicates by coloring the whole node / egde Changes of
	 * attributes are indicated by info nodes Green / ++: creation or change Red /
	 * --: deletion
	 */
		...
		if (!localStateManager.getCurrentState().getChildren().isEmpty()) {
			...
			for (EObject deletedNode : nextRuleState.getStructuralDelta().getDeletedObjects()) {
				if (!nodeBlacklist.contains(deletedNode)) {
					nodeMap.get(deletedNode).setAttribute("ui.style",
							"fill-color: rgba(150,0,0,255); text-background-color: rgba(255,0,0,128);");
					nodeMap.get(deletedNode).setAttribute("ui.label",
							nodeMap.get(deletedNode).getAttribute("ui.label") + " --");
				}
			}
	...
	/**
	 * Deletes all objects created to show the apply of the next rule and reverts
	 * the coloring
	 */
	...
	private void highlightMatch(IBeXMatch iBeXMatch) {
		...
		matchHiglightNode.setAttribute("ui.style",
				"fill-color: orange;" + "fill-mode: dyn-plain;" + "size: 2px;" + "shape: box;"
		...
		for (ComplexParameter param : iBeXMatch.getParameters().stream()
				.filter(param -> param instanceof ComplexParameter).map(param -> (ComplexParameter) param)
				.collect(Collectors.toList())) {
			try {
				EObject matchNode = param.getValue();
				if (!nodeBlacklist.contains(matchNode)) {
					nodeMap.get(matchNode).setAttribute("ui.style",
							"shadow-mode: gradient-radial; shadow-width: 10px; shadow-color: orange;");
					Edge matchHighlightEdge = graph.addEdge(Integer.toString(edgeID), matchHiglightNode,
							nodeMap.get(matchNode));
					matchHighlightEdge.setAttribute("ui.label", "");
					matchHighlightEdge.setAttribute("ui.style", "fill-color: white;" + "stroke-mode: dashes;"
							+ "stroke-color: black;" + "stroke-width: 2.0px;");
	...
	/**
	 * Reverts coloring of all graph objects Does not revert highlighting of matches
	 * (shadow of nodes)
	 */
		...
		if (!nodeBlacklist.contains(targetNode)) {
			if (!infoNodes.keySet().contains(targetNode)) {
				...
				addedNode.setAttribute("ui.style",
						"fill-color: grey;" + "shape: circle;" + "text-background-color: grey;" + "stroke-mode: plain;"
								+ "stroke-color: rgba(0,150,0,255);" + "stroke-width: 2.0px;");
				addedNode.setAttribute("ui.label", "++");

				addedEdge.setAttribute("ui.style", "fill-color: white;" + "stroke-mode: dashes;"
						+ "stroke-color: black;" + "stroke-width: 2.0px;");
		...
		if (!nodeMap.keySet().contains(node)) {
			...
			if (node.eClass().getInstanceClassName().indexOf(".") != -1)
				graph.getNode(Integer.toString(nodeID)).setAttribute("ui.label", node.eClass().getInstanceClassName()
						.substring(node.eClass().getInstanceClassName().indexOf(".") + 1));
			else
				graph.getNode(Integer.toString(nodeID)).setAttribute("ui.label", node.eClass().getInstanceClassName());
			// Increment nodeID
			nodeID++;
	...
	private Edge createGraphEdge(Link newLink) {
		// No check with edgeMap because different Link-objects can have same target and
		// source
		...
		if (nodeBlacklist.contains(newLink.getSrc()) || nodeBlacklist.contains(newLink.getTrg())) {
			...
			Edge createdEdge = graph.addEdge(Integer.toString(edgeID), nodeMap.get(newLink.getSrc()),
					nodeMap.get(newLink.getTrg()), true);
			if (createdEdge != null) {
	...
	private boolean equalEdge(Link one, Link two) {
		return (one.getSrc().equals(two.getSrc()) && one.getTrg().equals(two.getTrg())
				&& one.getType().equals(two.getType()));
	}

	/**
	 * Jumps to next state where pattern count in comparison to current pattern
	 * count changes
	 * 
