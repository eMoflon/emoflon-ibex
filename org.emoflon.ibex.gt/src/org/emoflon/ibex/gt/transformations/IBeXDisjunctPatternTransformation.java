...
public class IBeXDisjunctPatternTransformation {
	...
	public IBeXDisjunctContextPattern transformToContextPattern(){
		...
		IBeXDisjunctContextPattern ibexPattern = IBeXPatternModelFactory.eINSTANCE.createIBeXDisjunctContextPattern();
	...
	private List<IBeXDependentInjectivityConstraints> transformInjectivityConstraints(List<IBeXContextPattern> subpatterns, List<IBexDisjunctInjectivityConstraint> constraints){
		...
		for(Entry<List<IBeXContextPattern>, List<IBexDisjunctInjectivityConstraint>> entry: constraintMap.entrySet()) {
			IBeXDependentInjectivityConstraints depententConstraint = IBeXPatternModelFactory.eINSTANCE.createIBeXDependentInjectivityConstraints();
	 ...
	 */
	private IBexDisjunctInjectivityConstraint transformConstraints(IBeXContextPattern pattern1, IBeXContextPattern pattern2, List<List<IBeXNode>> nodes) {
		IBexDisjunctInjectivityConstraint constraint = IBeXPatternModelFactory.eINSTANCE.createIBexDisjunctInjectivityConstraint();
	 ...
	 */
	private List<IBeXDependentDisjunctAttribute> transformAttributeConstraints(Map<Set<IBeXNode>, IBeXContextPattern> dividedSubpatterns, List<IBeXConstraint> constraints,
			List<IBeXDependentInjectivityConstraints> injectivityConstraints) {
		...
		List<IBeXDisjunctAttribute> disjunctAttributes = constraints.stream().map(c -> transformAttributeConstraint(dividedSubpatterns, c)).collect(Collectors.toList());
		for(IBeXDisjunctAttribute attr: disjunctAttributes) {
		...
		List<IBeXDependentDisjunctAttribute> attrList = new ArrayList<IBeXDependentDisjunctAttribute>();
		//create the pool of dependent constraints
		for(Entry<Set<IBeXContextPattern>, List<IBeXDisjunctAttribute>> dependentAttribute: constraintMap.entrySet()) {
			IBeXDependentDisjunctAttribute attr = IBeXPatternModelFactory.eINSTANCE.createIBeXDependentDisjunctAttribute();
			attr.getDependentPatterns().addAll(dependentAttribute.getKey());
			attr.getAttributes().addAll(dependentAttribute.getValue());
			
			//search for the injectivity constraints
			List<IBeXDependentInjectivityConstraints> injecConstr = injectivityConstraints.stream().filter(i -> {
				for(IBeXContextPattern pattern: i.getPatterns()) {
					if(attr.getDependentPatterns().contains(pattern)) return true;
				}
				return false;
			}).collect(Collectors.toList());
	...
	/**
	 * transform a single disjunct attribute constraint 
	 */
	private IBeXDisjunctAttribute transformAttributeConstraint(Map<Set<IBeXNode>, IBeXContextPattern> dividedSubpatterns, IBeXConstraint constraint) {
		
		IBeXDisjunctAttribute disjunctAttribute = IBeXPatternModelFactory.eINSTANCE.createIBeXDisjunctAttribute();
		...
		disjunctAttribute.getSourcePattern().addAll(sourcePatterns);
		disjunctAttribute.getTargetPattern().addAll(targetPatterns);
		disjunctAttribute.getDisjunctAttribute().add(constraint);
