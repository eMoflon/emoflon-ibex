...
public class IBeXDisjointPatternFinder {
...
public IBeXDisjointPatternFinder(IBeXContext pattern) {
	
	allSubgraphs = new HashSet<Set<IBeXNode>>();
	if(pattern instanceof IBeXContextAlternatives) {
		throw new IllegalArgumentException("alternative patterns can not be disjoint");
	...
	currentSubgraphs = checkDisjointPatterns(thisPattern);
 ...
 * @return
 */
private void findDisjointSubpatterns(final Map<IBeXNode, Pair<Set<IBeXNode>, Boolean>> flattenedGraph, final List<IBeXNode> signatureNodes) {
 ...
 */
private Set<Set<IBeXNode>> checkDisjointPatterns(final IBeXContextPattern pattern) {
