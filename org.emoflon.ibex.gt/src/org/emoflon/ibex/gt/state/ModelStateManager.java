...
public class ModelStateManager {
	...
	private IBeXGTPatternMatcher<?> engine;
	...
	public ModelStateManager(final Resource model, final Resource trashResource, final IBeXGTPatternMatcher<?> engine,
			Map<String, Collection<IMatch>> matches, boolean forceNewStates, boolean loaded) {
		this.model = model;
	...
	public IMatch IBeXMatchToIMatchByList(RuleState state) {
		...
		for (IMatch imatch : matchesForPattern) {
			if (imatch.getObjects()
					.containsAll(parameter.values().stream().filter(param -> param instanceof ComplexParameter)
							.map(param -> ((ComplexParameter) param).getValue()).collect(Collectors.toSet())))
				return imatch;
		}
				...
				try {
					...
					try {
						...
						try {
							...
							for (EEnum enm : ePack.getEClassifiers().stream().filter(cls -> cls instanceof EEnum)
									.map(enm -> (EEnum) enm).collect(Collectors.toList())) {
								try {
				...
				for (Parameter paramSecond : second.getParameters()) {
					if (paramFirst instanceof ComplexParameter fP && paramSecond instanceof ComplexParameter sP) {
						if (paramFirst.getName().equals(paramSecond.getName())
								&& !(fP.getValue().equals(sP.getValue()))) {
							return false;
						}
	...
	public Optional<IMatch> addNewState(final GTRule rule, final IMatch match, final Map<String, Object> parameter,
		...
		rule.getAttributeAssignments().stream().filter(asgn -> match.isInMatch(asgn.getNode().getName()))
		...
		Set<EObject> nodesToWatch = findAllDeletedNodes(match, rule.getDeletion());

		// Find all nodes existing outside EObject containments but still within the
		// resource,
		// which will be placed into proper containment after rule application.
		Set<EObject> nodesInResource = findAllNodesRemovedFromResourceContainment(match, rule.getCreation());
		...
		// Error description: states are generated in parallel not in consecutive order
		// because
		...
		rule.getAttributeAssignments().stream().forEach(asgn -> {
		...
		if (addDeleteDelta(delta, match, rule.getDeletion()) | addCreateDelta(delta, comatch, rule.getCreation())) {
	...
	public Optional<IMatch> revertToPrevious() {
		...
		if (currentRuleState.getStructuralDelta() != null) {
			...
			// Restore deleted nodes and edges -> restoring edges is enough, since deleted
			// nodes are nodes whose containment edge has been removed
			LinkedList<Link> toRestore = new LinkedList<>();
			toRestore.addAll(delta.getDeletedLinks());
			// Ignore contents of the delete pattern (which is incomplete anyways) -> take
			// the ground truth from collected emf notifications
			...
			Set<EMFEdge> toDelete = delta.getCreatedLinks().stream()
					.map(link -> new EMFEdge(link.getSrc(), link.getTrg(), link.getType())).collect(Collectors.toSet());

			if (engine.getEngineProperties().needs_trash_resource())
...
//			// Restore Resource to EObject links
		...
		currentRuleState.getAttributeDeltas().stream().filter(atrDelta -> !createdNodes.contains(atrDelta.getObject()))
				.forEach(atrDelta -> {
					atrDelta.getObject().eSet(atrDelta.getAttribute(), getObjectFromValue(atrDelta.getOldValue()));
				});
	...
	private void addDeltaMatches(RuleState state, Map<String, Collection<IMatch>> addedMatches,
			Map<String, Collection<IMatch>> removedMatches) {
	...
	private Optional<IMatch> moveToChildState(final State childState) {
		...
		// Fix creation of new nodes -> otherwise new nodes will not be identical to
		// nodes created in previous iterations
		...
		if (delta != null) {
			...
			rs.getRule().getCreation().getNodes().forEach(node -> {
			...
			rs.getRule().getCreation().getEdges().forEach(edge -> {
				EObject src = (EObject) trueComatch.get(edge.getSource().getName());
				EObject trg = (EObject) trueComatch.get(edge.getTarget().getName());
		...
		rs.getAttributeDeltas().stream()
				.filter(atrDelta -> atrDelta.getObject()
						.eGet(atrDelta.getAttribute()) != getObjectFromValue(atrDelta.getNewValue()))
				.filter(atrDelta -> atrDelta.getObject()
						.eGet(atrDelta.getAttribute()) != getObjectFromValue(atrDelta.getNewValue()))
				.forEach(atrDelta -> {
					atrDelta.getObject().eSet(atrDelta.getAttribute(), getObjectFromValue(atrDelta.getNewValue()));
	...
	private AttributeDelta createAttributeDelta(final IMatch match, final IBeXAttributeAssignment assignment) {
		...
		delta.setAttribute(assignment.getAttribute());
		EObject node = (EObject) match.get(assignment.getNode().getName());
		delta.setObject(node);

		EDataType type = assignment.getAttribute().getEAttributeType();
		oldVal.setType(type);
		try {
			oldVal.setValueAsString(valueToString(factory, type, node.eGet(assignment.getAttribute())));
		} catch (IOException e) {
	...
	private AttributeDelta completeAttributeDelta(final IMatch match, final IBeXAttributeAssignment assignment,
		...
		newVal.setType(type);
		try {
			newVal.setValueAsString(valueToString(factory, type, node.eGet(assignment.getAttribute())));
		} catch (IOException e) {
			// Auto-generated catch block
			e.printStackTrace();
		}
	...
	private boolean addCreateDelta(StructuralDelta delta, final IMatch match, final IBeXRuleDelta pattern) {
		if (pattern.getNodes().size() == 0 && pattern.getEdges().size() == 0) {
			return false;
		}

		for (IBeXNode createdNode : pattern.getNodes()) {
			delta.getCreatedObjects().add((EObject) match.get(createdNode.getName()));
		}

		for (IBeXEdge createdEdge : pattern.getEdges()) {
			EObject src = (EObject) match.get(createdEdge.getSource().getName());
			EObject trg = (EObject) match.get(createdEdge.getTarget().getName());
	...
	private boolean addDeleteDelta(StructuralDelta delta, final IMatch match, final IBeXRuleDelta pattern) {
		if (pattern.getNodes().size() == 0 && pattern.getEdges().size() == 0) {
			return false;
		}

		for (IBeXNode deleteNode : pattern.getNodes()) {
		...
		for (IBeXEdge deletedEdge : pattern.getEdges()) {
			EObject src = (EObject) match.get(deletedEdge.getSource().getName());
			EObject trg = (EObject) match.get(deletedEdge.getTarget().getName());
			Link link = factory.createLink();
			link.setSrc(src);
			link.setTrg(trg);
	...
	private Set<EObject> findAllDeletedNodes(final IMatch match, final IBeXRuleDelta pattern) {
		...
		for (IBeXEdge deletedEdge : pattern.getEdges()) {
			if (deletedEdge.getType().isContainment()) {
				EObject current = (EObject) match.get(deletedEdge.getTarget().getName());
	...
	private Set<EObject> findAllNodesRemovedFromResourceContainment(final IMatch match, final IBeXRuleDelta pattern) {
		Set<EObject> createdNodes = new HashSet<>(pattern.getNodes());
		...
		for (IBeXEdge createdEdge : pattern.getEdges()) {
			...
			if (createdEdge.getType().isContainment() && trg.eContainer() == null && !createdNodes.contains(trg)
					&& model.getContents().contains(trg)) {
	...
	private static Object stringToValue(final EFactory factory, final EDataType atr, final String value)
			throws Exception {
	...
	private static String valueToString(final EFactory factory, final EDataType atr, final Object value)
			throws IOException {
		EcorePackage epack = EcorePackage.eINSTANCE;
		EFactory efac = epack.getEFactoryInstance();
		if (atr == epack.getEString()) {
