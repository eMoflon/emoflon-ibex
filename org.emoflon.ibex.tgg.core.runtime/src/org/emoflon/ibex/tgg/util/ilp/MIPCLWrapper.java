...
final class MIPCLWrapper extends ILPSolver {
	...
	static {
		try {
			MIPCLWrapper.addLibraryPath("C:\\Program Files\\PNN\\MIPCL\\mip\\bin");
		} catch (Exception e) {
	...
	private final IntToObjectMap<Var> variableIdToMIPCLVar = CollectionFactory.cfactory.createIntToObjectHashMap();
	 ...
	 */
	MIPCLWrapper(final ILPProblem ilpProblem, final boolean onlyBinaryVariables) {
		super(ilpProblem);
	...
	public ILPSolution solveILP() throws Exception {
		...
		currentTimeout = MIPCLWrapper.MIN_TIMEOUT + (long) Math.ceil(Math.pow(1.16, Math.sqrt(currentTimeout)));
		if (currentTimeout < 0) {
			currentTimeout = MIPCLWrapper.MAX_TIMEOUT;
		}
		currentTimeout = Math.min(currentTimeout, MIPCLWrapper.MAX_TIMEOUT);
	...
	private void prepareModel() {
		this.solver = new MIPshell("ILP problem");
	 ...
	 * @return the result
	 */
	private void solveModel(final long timeout) {
		ILPSolver.logger.debug("Solving problem with time limit " + timeout + " seconds");
		this.solver.optimize(false, timeout);
	}
	...
	private void registerVariable(final String variable, final int variableID) {
		...
		if (this.onlyBinaryVariables) {
			var = new Var(this.solver, variable, MIPshell.VAR_BIN, 0, 1);
		} else {
			var = new Var(this.solver, variable, MIPshell.VAR_INT, -LPshell.VAR_INF, LPshell.VAR_INF);
		}
	...
	private void registerConstraint(final ILPConstraint constraint) {
		LinSum linSum = this.createLinSum(constraint.getLinearExpression());

		switch (constraint.getComparator()) {
			case ge -> this.solver.addCtr(linSum, constraint.getValue(), LPshell.INF);
			case le -> this.solver.addCtr(linSum, -LPshell.INF, constraint.getValue());
			case eq -> this.solver.addCtr(linSum, constraint.getValue(), constraint.getValue());
	...
	private void registerObjective(final ILPObjective objective) {
		LinSum linSum = this.createLinSum(objective.getLinearExpression());

		switch (objective.getObjectiveOperation()) {
			case maximize -> this.solver.maximize(linSum);
			case minimize -> this.solver.minimize(linSum);
			default -> throw new IllegalArgumentException("Unsupported operation: " + objective.getObjectiveOperation().toString());
		}
	}

	private LinSum createLinSum(final ILPLinearExpression linearExpression) {
		...
		for (int variableId : linearExpression.getVariables()) {
			...
			linSum.add(coefficient, this.variableIdToMIPCLVar.get(variableId));
		}
		return linSum;
	}
}
