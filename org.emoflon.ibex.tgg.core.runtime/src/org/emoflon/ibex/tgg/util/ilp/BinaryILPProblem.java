...
public class BinaryILPProblem extends ILPProblem {
	...
	private class Implication {
		...
		private void fixImplVariable(int id, boolean choice) {
			if(leftVariable == id) {
				if(!choice) { //all other vars irrelevant 
					isRelevant = false;
				}
			}
		...
		public String toString() {
			...
			return "Implication("+name+")"
				+ getVariable(leftVariable) + " -> "
	...
	private class NegativeImplication {
		...
		public String toString() {
			...
			return "NegativeImplication("+name+")"
				+ "-("+String.join(" V ", termStringsLeft) + ") -> "
	...
	private void generateContraints() {
		...
		for(it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<ObjectLinkedOpenHashSet<Implication>> implOfSameVar : implications.int2ObjectEntrySet()) {
			ILPLinearExpression expr = createLinearExpression();
	...
	private void applyFixedVariablesToImplications(int[] fixedVariables) {
		ObjectIterator<it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<ObjectLinkedOpenHashSet<Implication>>> implMapIt = implications.int2ObjectEntrySet().iterator();
		while(implMapIt.hasNext()) {
			it.unimi.dsi.fastutil.ints.Int2ObjectMap.Entry<ObjectLinkedOpenHashSet<Implication>> entry = implMapIt.next();
	...
	public void addImplication(String leftSide, Collection<String> rightSide, String name) {
		...
		impl.rightVariables = new IntOpenHashSet(rightSide.stream().map(s -> getVariableId(s)).collect(Collectors.toList()));
	...
	public void addExclusion(Collection<String> variables, String name) {
		...
		excl.name = name;
		exclusions.add(excl);
		excl.variableGroup = new IntOpenHashSet(variables.stream().map(s -> getVariableId(s)).collect(Collectors.toList()));
		for(Entry id: this.getFixedVariableValues().int2IntEntrySet()) {
	...
	public void addNegativeImplication(Collection<String> leftSide, Collection<String> rightSide, String name) {
		...
		impl.leftVariables = new IntOpenHashSet(leftSide.stream().map(s -> getVariableId(s)).collect(Collectors.toList()));
		impl.rightVariables = new IntOpenHashSet(rightSide.stream().map(s -> getVariableId(s)).collect(Collectors.toList()));
		for(Entry id: this.getFixedVariableValues().int2IntEntrySet()) {
			impl.fixImplVariable(id.getIntKey(), id.getIntValue() == 1);
		}
