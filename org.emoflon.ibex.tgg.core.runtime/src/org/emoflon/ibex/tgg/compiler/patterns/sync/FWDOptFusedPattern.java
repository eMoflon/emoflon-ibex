...
public class FWDOptFusedPattern extends BasicSyncPattern {
	private TGGComplementRule flattenedComplementRule;

	public FWDOptFusedPattern(BlackPatternFactory factory) {
		super(factory);
	...
	protected void initialise() {		
		String name = setFusedName(flattenedComplementRule.getName(), flattenedComplementRule.getKernel().getName()) + PatternSuffixes.FWD_OPT;
	...
	protected void createPatternNetwork(BlackPatternFactory factory) {
		...
		addPositiveInvocation(factory.getFactory(flattenedComplementRule.getKernel()).createBlackPattern(FWDOptBlackPattern.class));		
