...
public class AddPrefix extends RuntimeTGGAttributeConstraint
{
    ...
    */
   public void solve(RuntimeTGGAttributeConstraintVariable prefix, RuntimeTGGAttributeConstraintVariable word, RuntimeTGGAttributeConstraintVariable result)
   {
      ...
      else if (bindingStates.equals("FBB"))
      {
         ...
         {
            prefix.bindToValue(((String) result.getValue()).substring(0, ((String) result.getValue()).length() - ((String) word.getValue()).length()));
            setSatisfied(true);
         }
      } 
