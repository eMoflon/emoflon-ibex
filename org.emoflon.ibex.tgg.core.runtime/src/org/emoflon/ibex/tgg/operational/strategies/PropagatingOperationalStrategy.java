...
public abstract class PropagatingOperationalStrategy extends OperationalStrategy {
	...
	public void terminate() throws IOException {
		...
		LoggerConfig.log(LoggerConfig.log_translationTime(),
				() -> "Translation time: " + df.format((double) translateTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_repairTime(),
				() -> "Repair time: " + df.format((double) repairTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_removalTime(),
				() -> "Remove time: " + df.format((double) removeTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "Match application time: "
				+ df.format((double) matchApplicationTime / (double) (1000 * 1000 * 1000)) + " -> {");
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "     Init time:            "
				+ df.format((double) initMatchApplicationTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "     Choose match time:    "
				+ df.format((double) chooseMatchTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "     Create elements time: " + df.format(
				(double) ((IbexGreenInterpreter) greenInterpreter).getCreationTime() / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "     Finalize time:        "
				+ df.format((double) finishRuleApplicationTime / (double) (1000 * 1000 * 1000)));
		LoggerConfig.log(LoggerConfig.log_matchApplicationTime(), () -> "}");
		LoggerConfig.log(LoggerConfig.log_collectMatchTime(), () -> "Match collection time: "
				+ df.format((double) matchDistributor.getTime() / (double) (1000 * 1000 * 1000)));
